...
//    int main() {
   int i,j,k,Ndatos1,Ndatos2,Ndatos1s2, perio;
    ...
    sprintf(salida,"correlaciones1.%s.dat",letras);
    */
 //   printf("OK\n");
    ...
    double *emg1;
    Ndatos1=filesize("emg1.Sound",1);
    Ndatos1s2=Ndatos1/2;
    emg1=dvector(1,Ndatos1);
    file_to_vector("emg1.Sound",emg1,1,Ndatos1,1,1);
    
    double *emg2;
    Ndatos2=filesize("ZF-MCV_2015-12-01_13_13_49_vs_29_band.Sound",1);
    emg2=dvector(1,Ndatos2);
    file_to_vector("ZF-MCV_2015-12-01_13_13_49_vs_29_band.Sound",emg2,1,Ndatos2,1,1);
  //  printf("\n%lg\n", emg2[1]);
   ...
   vector_to_file("hilbert.emg1.dat",hilb1,1,Ndatos1);
   ...
   printf("1ok\n");  
    ...
    vector_to_file("hilbert.vscomp.dat",hilb2,1,Ndatos2);

    printf("2ok\n");  
	emg2M=emg2[1];
   	for(i=2;i<Ndatos2;i++){
		if(emg2M<fabs(emg2[i])){
			 emg2M=fabs(emg2[i]);
		}
	}
    printf("3ok\n");  
   ...
   double v1[1],v2[1],dt, t;
	double rk4emg2M;
   double *av_sound1,*av_sound2,*emg1postrk4;
   av_sound1=dvector(1,Ndatos1);
   av_sound2=dvector(1,Ndatos2);
   emg1postrk4=dvector(1,Ndatos2);
    k=0;
   dt=1/10000.;
   aa.tau=15./1500.;
   //aa.tau=.5/1500.;
   for(i=1;i<=Ndatos1;i++){
    ...
    printf("4ok\n");  
	rk4emg2M=av_sound2[1];
   	for(i=2;i<Ndatos2-1;i++){
		if(rk4emg2M<fabs(av_sound2[i])){
			 rk4emg2M=fabs(av_sound2[i]);
		}
	} 
    printf("5ok\n");
	emg1postrk4[1]=av_sound2[1];/*
	for(i=2;i<Ndatos2;i++){
		emg1postrk4[i]=av_sound2[i];//*emg2M/rk4emg2M;
	}
  */
   vector_to_file("senalpostrk4.dat",av_sound2,1,Ndatos2);
   vector_to_file("emg1postrk4.dat",av_sound1,1,Ndatos1s2);
   ...
   c2=vector(1,Ndatos2); data2=vector(1,Ndatos2); ans2=vector(1,2*Ndatos2);
   //c2=vector(1,POT2); data2=vector(1,POT2); ans2=vector(1,2*POT2);

   double *sav1,*sav2, *savnorm2,savmax;
   sav1=dvector(1,Ndatos1);
   sav2=dvector(1,Ndatos2);
   savnorm2=dvector(1,Ndatos2);
   ...
   vector_to_file("envolventeemg1.dat",sav1,1,Ndatos1s2);
   
   printf("envolventeemg1es: %s\n","envolventeemg1.dat");
    // segunda

    // LOS FOR LLEGABAN HASTA POT2, LOS CAMBIO POR Ndatoss2 
    //for(i=1;i<=POT2;i++) data2[i]=(float) av_sound2[i];
	for(i=1;i<=Ndatos2;i++) data2[i]=(float) av_sound2[i];
    savgol(c2,513,256,256,0,4);
	printf("savgol ok\n");
    //for(index=1;index<=POT2;index++) data2[index]=fabs(data2[index]);
	for(index=1;index<=Ndatos2;index++) data2[index]=fabs(data2[index]);
	printf("fabs ok\n");
	//convlv(data2,POT2,c2,513,1,ans2);
    convlv(data2,Ndatos2,c2,513,1,ans2);
    printf("convlv ok\n");
    for(i=2;i<Ndatos2-1;i++) sav2[i]=(double) ans2[i];
	// for(i=2;i<POT2-1;i++) sav2[i]=(double) ans2[i];
	//cambie los pot2 hasta aca
	/*/agrego desde aca	
	savmax=sav2[1];
	for(i=2;i<POT2-1;i++){
		if(savmax<fabs(sav2[i]){
			savmax=sav2[i];
			}
		}
	for(i=1;i<POT2-1;i++){
		savnorm2[i]=sav2[i]*emg2M/savmax;
		}
	//hasta aca */    
    vector_to_file("envolventesenal.dat",sav2,1,Ndatos2); 
    printf("envolventeseñal: %s\n","envolventesenal.dat");
    //calculo de la correlacion entre las señales.
    
    Nmin=Ndatos1s2;
    
    FILE *ptr;
    ptr=fopen("convolucionemg1.dat","w");
	int testeo;
	testeo=1;
    for(j=2;j<Ndatos2-Nmin;j++){
    	...
    	fprintf(ptr,"%f\t %i\n",r,j);
        if(r>0.8){pFile=fopen("correlacionabienemg1.dat","a");
            //fprintf(pFile,"%f\t %d\t %s\t %d\n",r,j,letras,perio);
            fprintf(pFile,"%f\t %i\n",r,j);
	    	fclose(pFile);
        	}
    }
    printf("convolucion: %s\n","convolucionemg1.dat");        
