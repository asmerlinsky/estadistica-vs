...
//    int main() {
 ...
 //   printf("OK\n");
    ...
    double *emg1;
    Ndatos1=filesize("emg2.Sound",1);
    Ndatos1s2=Ndatos1/2;
    emg1=dvector(1,Ndatos1);
    file_to_vector("emg2.Sound",emg1,1,Ndatos1,1,1);
  ...
  //  printf("\n%lg\n", emg2[1]);
   ...
   vector_to_file("hilbert.emg2.dat",hilb1,1,Ndatos1);
   ...
   vector_to_file("envolventeemg2.dat",sav1,1,Ndatos1s2);
   
   printf("envolventeemg2es: %s\n","envolventeemg2.dat");
    // segunda
    ...
    FILE *ptr;
    ptr=fopen("convolucionemg2.dat","w");
	int testeo;
	testeo=1;
    for(j=2;j<Ndatos2-Nmin;j++){
    	...
    	fprintf(ptr,"%g\t %d\n",r,j);
        if(r>0.8){pFile=fopen("convolucionemg2.dat","a");
